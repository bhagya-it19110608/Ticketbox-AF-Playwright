...
test('Ticketbox QA Resale Load', async ({browser})=>
{
    //chrome plugins
   ...
   await signup.click();
   await expect (page).toHaveURL('https://ticketbox\.lk/signup');
   ...
   await signin.click();
   await expect (page).toHaveURL('https://ticketbox\.lk/login');
   ...
   console.log (await page.title());
   //await expect(page).toHaveTitle("TicketBox Reselling - Sri Lanka's First and Premium Event Tickets Reselling Platform");
   /* Bug for the title as actual - TicketBox - Sri Lanka's Premium Event Tickets Platform / server pointing issue */
});
